# Complete Cleanup Function for main.py

Replace the `cleanup_duplicate_assignments` function (around line 219) with this complete implementation:
...
## Missing Lines to Fill In:

**Line 234:** `continue  # Skip auto-generated assignments`

**Line 239:** `groups[key] = []`

**Line 248:** `return 0`

**Lines 249-250:** `import re` (on line 249)
...
**Lines 266-267:** `kept_count += len(docs_list)` and `continue`

**Lines 276-291:** The entire loop body for finding widest doc (see complete code above)

**Line 295:** `widest_doc = docs_list[0][0]`

**Lines 302-305:** The delete loop body (see complete code above)

**Lines 316-317:** `import traceback` (on line 316)

## Why This is Still Needed:

Even though you've removed the manual assignment logic from the scheduler, you may still have **old duplicate manual assignments in Firestore** from previous testing. This cleanup endpoint will remove those duplicates so they don't cause conflicts.

Once cleaned up, the automatic scheduler will work without overlap errors.
